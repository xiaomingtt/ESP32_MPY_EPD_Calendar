...
# 文件路径
INPUT_IMAGE_PATH = Path(r"C:\Users\Administrator\Desktop\20230530150217.jpg")  
...
def three_color_dither(img: Image.Image) -> Image.Image:
    """红黑白三色屏专属Floyd-Steinberg抖动算法，模拟渐变效果"""
    img_np = np.array(img, dtype=np.float32)
    height, width = img_np.shape[:2]

    for y in range(height):
        for x in range(width):
            old_r, old_g, old_b = img_np[y, x]
            lum = calculate_luminance(old_r, old_g, old_b)
            _, a, _ = rgb2lab(old_r, old_g, old_b)

            # 找到最接近的三色目标值
            # 优先级：红色 > 黑色 > 白色
            is_red = (
                (RED_HUE_RANGE1[0] <= get_hue(old_r, old_g, old_b) <= RED_HUE_RANGE1[1] or
                 RED_HUE_RANGE2[0] <= get_hue(old_r, old_g, old_b) <= RED_HUE_RANGE2[1])
                and a >= RED_A_CHANNEL_MIN
                and (old_g + old_b) < WHITE_GB_THRESHOLD
            )

            if is_red:
                new_r, new_g, new_b = 255, 0, 0
            elif lum < BLACK_LUMINANCE_THRESHOLD:
                new_r, new_g, new_b = 0, 0, 0
            else:
                new_r, new_g, new_b = 255, 255, 255

            # 计算误差
            err_r = (old_r - new_r) * DITHER_STRENGTH
            err_g = (old_g - new_g) * DITHER_STRENGTH
            err_b = (old_b - new_b) * DITHER_STRENGTH

            # 误差扩散到相邻像素
            if x + 1 < width:
                img_np[y, x+1] += [err_r * 7/16, err_g * 7/16, err_b * 7/16]
            if x - 1 >= 0 and y + 1 < height:
                img_np[y+1, x-1] += [err_r * 3/16, err_g * 3/16, err_b * 3/16]
            if y + 1 < height:
                img_np[y+1, x] += [err_r * 5/16, err_g * 5/16, err_b * 5/16]
            if x + 1 < width and y + 1 < height:
                img_np[y+1, x+1] += [err_r * 1/16, err_g * 1/16, err_b * 1/16]

            # 赋值新像素
            img_np[y, x] = [new_r, new_g, new_b]
