...
# ==============================================  核心配置区（所有可调参数都在这里） ==============================================
# 屏幕分辨率（严格对应SSD1683 400*300规格，不可随意修改）
EPD_WIDTH = 400
EPD_HEIGHT = 300
...
# 图片缩放适配模式
# "stretch": 拉伸铺满全屏（原代码默认，无黑边）
# "crop": 等比例缩放+居中裁剪（无变形，裁掉超出部分）
# "pad": 等比例缩放+留白填充（无变形，空白处填白色）
RESIZE_MODE = "stretch"
...
def main():
    ...
    print("="*60)
    print(f"SSD1683 400*300 红黑白墨水屏图片处理工具")
    print("="*60)
    ...
    try:
        ...
        if EPD_WIDTH <= 0 or EPD_HEIGHT <= 0:
            raise ValueError(f"分辨率必须大于0，当前: {EPD_WIDTH}*{EPD_HEIGHT}")
